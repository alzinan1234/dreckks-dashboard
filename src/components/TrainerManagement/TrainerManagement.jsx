...
const TrainerManagement = () => {
  ...
  // Modified handler to navigate to the dynamic page
  const handleViewDetails = (trainerId) => {
    router.push(`/admin/trainer-management/${trainerId}`);
  };
  ...
  return (
    <>
      <div className="bg-[#343434] p-6 rounded-lg text-white">
          ...
          <table className="min-w-full border-separate border-spacing-y-4">
            ...
            <tbody className="border-b border-gray-600">
              {filteredTrainers.map((trainer, index) => (
                <tr key={index} className="border-b border-gray-600 text-sm text-white py-5">
                  <td className="py-2 px-4 text-center border-b border-gray-600">{trainer.id}</td>
                  <td className="py-2 px-4 text-center flex items-center justify-center gap-2 border-b border-gray-600">
                    <Image
                      src={trainer.avatar}
                      alt="Trainer Avatar"
                      width={24}
                  ...
                  <td className="py-2 px-4 text-center border-b border-gray-600">
                    <div className="flex justify-center gap-2 ">
                      {/* View Button (purple circle with eye) */}
                      <button
                        onClick={() => handleViewDetails(trainer.id)} // Pass only the ID now
                        className="w-8 h-8 rounded-full flex items-center justify-center transition-colors"
                      >
                        <Image src="/icon/eye.svg" alt="View" width={25} height={25} />
                      </button>
                      {/* Edit Button (orange circle with pencil) */}
        ...
        {/* TrainerDetailsModal is removed as it's no longer needed here */}
      </div>
